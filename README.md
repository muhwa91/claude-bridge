...
| **음악 재생** | 음성채널에서 유튜브 재생목록을 셔플 재생(`ㅁ노래`·`ㅁ정지`·`ㅁ다음`) + 곡 지정 재생 `ㅁ재생 <제목>` · 목록 추가/제거 `ㅁ추가`/`ㅁ삭제`. 곡이 바뀔 때마다 채널에 제목을 알린다 |
...
| `MUSIC_PLAYLIST_ID` | `ㅁ노래` 재생 · `ㅁ추가`/`ㅁ삭제` 대상 유튜브 재생목록 **ID**(URL 의 `list=` 뒤). 비우면 `ㅁ노래` 는 미설정 안내 |
