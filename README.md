...
- **아키텍처**: `docs/개발/아키텍처.md`
- **의사결정(ADR)**: `docs/개발/의사결정/` — 단일루프·stdin 인젝션 방어·경로별 도구 권한·stream-json
- **어댑터 계층**: `docs/기능/디스코드_이관/` — 어댑터 설계·계약·UX

> 위 문서들은 **작업 레포에만** 있다(공개 미러 큐레이션 제외 대상). 그래서 **링크를 걸지 않는다** — 걸면 공개 저장소에서 404 가 된다.
