...
class Adapter(Protocol):
    ...
    def play_music(self, channel_id: int, user_id: int, query: str = "") -> str:
        ...
        🔴 **빈 문자열 = 코어는 send 하지 않는다**(play/stop/skip 공통 계약). 어댑터가 이미
        보냈거나 보낼 것이 없다는 뜻이다 — 회신을 곡 전환 알림보다 **먼저** 내보내려면 재생을
        시작하는 어댑터 쪽이 직접 보내야 하기 때문(코어는 반환 뒤에야 send 할 수 있다).
    ...
    def stop_music(self, channel_id: int) -> str:
        """음성재생 정지 + 음성채널 퇴장(디스코드 전용). 회신(""=미발송). 타 어댑터 미지원."""
        ...
